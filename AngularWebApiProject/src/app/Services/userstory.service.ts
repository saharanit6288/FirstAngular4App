...
export class UserStoryService {
    ...
    constructor(private auth: AuthenticationService, private _http: Http, private globals: Globals) {
        ...
        this.pathUrl = globals.BASE_USER_ENDPOINT + 'api/userstories/';
    }
    ...
    getByPaging(page: number, pagesize: number): Observable<any> {
        let headers = new Headers();
        let url = this.pathUrl + 'GetUserStoriesByPaging?page=' + page + '&pagesize=' + pagesize;
        headers.append('Authorization', 'Bearer ' + this.token);
        let options = new RequestOptions({ headers: headers });
        return this._http.get(url, options)
            .map((response: Response) => <any>response.json())
    ...
    getUserStoriesDDL(): Observable<any> {
        let headers = new Headers();
