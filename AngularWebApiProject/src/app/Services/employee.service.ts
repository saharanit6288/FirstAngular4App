...
export class EmployeeService {
    ...
    constructor(private auth: AuthenticationService, private _http: Http, private globals: Globals) {
        ...
        this.pathUrl = globals.BASE_USER_ENDPOINT + 'api/employees/';
    }
	
	getEmployeeAutoCompleteUrl():string{
		return this.pathUrl;
	}

    get(term: string, page: number, pagesize: number): Observable<any> {
        let headers = new Headers();
        let url = this.pathUrl + '?q=' + term + '&page=' + page + '&pagesize=' + pagesize;
    ...
    getByPaging(page: number, pagesize: number): Observable<any> {
        let headers = new Headers();
        let url = this.pathUrl + 'GetEmployeesByPaging?page=' + page + '&pagesize=' + pagesize;
        headers.append('Authorization', 'Bearer ' + this.token);
        let options = new RequestOptions({ headers: headers });
        return this._http.get(url, options)
            .map((response: Response) => <any>response.json())
    ...
    getEmployeesDDL(): Observable<any> {
        let headers = new Headers();
        let url = this.pathUrl + 'GetAllForDdl';
        headers.append('Authorization', 'Bearer ' + this.token);
        let options = new RequestOptions({ headers: headers });
        return this._http.get(url, options)
            .map((response: Response) => <any>response.json())
